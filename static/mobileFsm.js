...
function crossBrowserRelativeMousePos(e) {
    ...
    console.log(mouse.y - document.querySelector('#canvas').getBoundingClientRect().top);
	return {
		'x': mouse.x - element.x,
		'y': mouse.y - document.querySelector('#canvas').getBoundingClientRect().top
	};
}
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        function doubleClick(x,y) {
		    selectedObject = selectObject(x, y - document.querySelector('#canvas').getBoundingClientRect().top);
            console.log(selectedObject);
		    if(selectedObject == null) {
                console.log(x,y);
			    selectedObject = new Node(x, y - document.querySelector('#canvas').getBoundingClientRect().top);
			    nodes.push(selectedObject);
			    resetCaret();
			    draw();
            } else if(selectedObject instanceof Node) {
		...
		document.getElementById("canvas").addEventListener('touchstart',onTouch);
		document.getElementById("canvas").addEventListener('touchend',onEndTouch);
		...
		function onEndTouch(ev){
			var currentTime = new Date().getTime();
			var tapLength = currentTime - lastTap;
			clearTimeout(timeout);
			if (tapLength < 500 && tapLength > 0) {
				console.log('Double Tap',ev.touches);
                doubleClick(lastTouch.clientX, lastTouch.clientY);
				ev.preventDefault();
			} else {
