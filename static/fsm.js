...
function sync(){
    if(selectedObject != null){
        if(selectedObject instanceof Link || selectedObject instanceof SelfLink){
                document.getElementById('transitions').value=selectedObject.text.replace(new RegExp(",", "g"), '\n'); 
                if(selectedObject instanceof SelfLink){
                    document.getElementById('transitionsheader').value=`State ${selectedObject.node.text.replace(new RegExp("_", "g"), '')} to state ${selectedObject.node.text.replace(new RegExp("_", "g"), '')}`; 
                }else{
                    document.getElementById('transitionsheader').value=`State ${selectedObject.nodeA.text.replace(new RegExp("_", "g"), '')} to state ${selectedObject.nodeB.text.replace(new RegExp("_", "g"), '')}`; 
                }
        }
    }
}
...
function Node(x, y) {
    ...
    this.text = 'q';
    this.links = {};
    let statename = numnodes.toString();
    for (i = 0; i < statename.length; i++) {
        this.text+='_'+statename.slice(i,i+1);
    }
    numnodes++;
}
...
function drawText(c, originalText, x, y, angleOrNull, isSelected) {
    ...
    //set text to ... if there is too much text
    if(text.length>6){
        text='...';
    }
...
function dfa(word){
    ...
    for( let i=0; i<links.length; i++ ){
        ...
        if(link instanceof Link){
            for( let j=0;j < link.text.split(',').length;j++){
                let kelet=link.text.split(',')[j];
                link.nodeA.links[kelet]=link.nodeB;
            }
        }
        else if(link instanceof SelfLink){
            for( let j=0;j < link.text.split(',').length;j++){
                let kelet=link.text.split(',')[j];
                link.node.links[kelet]=link.node;
            }
        }
     }
    ...
    for(let i=0;i< word.length;i++){
        let st = word[i];
        if(!(st in currNode.links)){
            return false;
        }
        currNode=currNode.links[st];
    }
     return currNode.isAcceptState;
}

function getlink(ch,node){
    let maavarim=node.links;
    for(let maavar in maavarim){
          if(maavar.split('/')[0]==ch){
                return maavar;
          }
    }
}
...
function turing(word){
    ...
    let seret = word.split('');   
    seret.unshift('|');    
    let currNode=nodes[0];
    let index=1;
    let st;
    if(index<seret.length){
        st=seret[index];
    }else{
        st="-";
    }
    let currkey=getlink(st,currNode);
    while(currkey!=undefined){
           let linkarr=currkey.split('/');     
           let write = linkarr[1];
           let move = linkarr[2];
           if(index<seret.length){
                seret[index] = write;
           ...
           currNode=currNode.links[currkey];
           currkey=getlink(st,currNode);
    }
     return currNode.isAcceptState;
}
...
document.addEventListener('DOMContentLoaded', () => {
        document.querySelector('#transitions').onkeyup = function(){
                if(selectedObject!=null){
                    if('text' in selectedObject){
                            selectedObject.text=this.value.replace(new RegExp('\n', "g"),",");
        ...
        document.querySelector('#run').onsubmit = () => {
                ...
                var runningfunction;
                if(document.getElementById('automaton').checked) {
                  runningfunction = dfa;
                }else if(document.getElementById('turing').checked) {
                  runningfunction = turing;
                }
                if(runningfunction(word)){
